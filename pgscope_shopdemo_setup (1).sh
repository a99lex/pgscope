...
if kubectl exec "$PRIMARY" -- psql -U postgres -d postgres -tAc \
  ...
  read -r -p "Delete and rebuild it? ALL DATA IN ${DATABASE} WILL BE LOST. [y/N] " REBUILD_DB
  case "$REBUILD_DB" in
    y|Y|yes|YES|Yes)
      kubectl exec "$PRIMARY" -- psql -v ON_ERROR_STOP=1 -U postgres -d postgres \
        -c "DROP DATABASE ${DATABASE} WITH (FORCE);"
      ;;
    *)
      echo "Keeping the existing database. Setup will only add missing objects."
      ;;
  esac
fi
...
case "$START_LOAD" in
  y|Y|yes|YES|Yes)
    ...
    LOAD_QUERY_1="SELECT COUNT(*) FROM order_items;"
    LOAD_QUERY_2="SELECT COUNT(*) FROM orders o JOIN order_items oi ON oi.order_id = o.order_id;"
    LOAD_QUERY_3="SELECT oi.product_id, SUM(oi.quantity) AS units_sold, SUM(oi.quantity * oi.unit_price) AS revenue FROM order_items oi GROUP BY oi.product_id ORDER BY revenue DESC;"
    ...
    while true; do
      ...
      kubectl exec "$PRIMARY" -- psql -q -U postgres -d "$DATABASE" -c "$LOAD_QUERY_3" >/dev/null
      sleep 1
    done
    ;;
  *)
